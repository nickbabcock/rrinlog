...
pub fn parse_nginx_line<'a>(text: &'a str) -> Result<NewLog<'a>> {
    ...
    if let Some(caps) = RE.captures(text) {
        Ok(NewLog {
            ...
            body_bytes_sent: caps.name("body_bytes_sent").unwrap().as_str().parse::<i32>().ok(),
...
mod tests {
    ...
    #[test]
    fn test_parse_date() {
        let expected = FixedOffset::west(5 * 3600).ymd(2017, 11, 3).and_hms(6, 49, 45);
    ...
    fn test_parse_nginx() {
        ...
        assert_eq!(NewLog {
            ...
            referer: Some("https://nbsoftsolutions.com/blog/monitoring-windows-system-metrics-with-grafana"),
            user_agent: Some("Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/61.0.3163.100 Safari/537.36"),
